...
function Step4({ data, onChange }: { data: FormData; onChange: (d: Partial<FormData>) => void }) {
  ...
  return (
    <div>
      ...
      <AccordionSection title={locale === 'en' ? '🎨 AI Watercolor — Premium' : '🎨 Aquarelle IA — Premium'}>
          <div style={{ textAlign: 'center', padding: '20px 12px' }}>
            <div style={{ display: 'inline-block', background: 'linear-gradient(135deg, #C9A84C, #e8c96a)', color: 'white', fontSize: 9, fontWeight: 800, letterSpacing: '0.15em', textTransform: 'uppercase', padding: '4px 12px', borderRadius: 9999, marginBottom: 14 }}>
              Coming soon
            </div>
            <div style={{ fontFamily: 'var(--font-playfair-display)', fontSize: 20, fontWeight: 700, color: '#3a3330', marginBottom: 6 }}>
              Pack Aquarelle IA
            </div>
            <div style={{ fontFamily: 'var(--font-cormorant-garamond)', fontStyle: 'italic', fontSize: 14, color: '#9a928a', marginBottom: 16, lineHeight: 1.5 }}>
              {locale === 'en'
                ? 'A unique watercolor generated by AI for each of your events, based on your real venues.'
                : 'Une aquarelle unique générée par l\u2019IA pour chacun de vos événements, inspirée de vos vrais lieux de réception.'}
            </div>
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginBottom: 16 }}>
              {['Hilton Tel Aviv', 'Château de Vaux', 'Synagogue Buffault'].map((ex, i) => (
                <div key={i} style={{ width: 70, height: 95, borderRadius: 8, background: 'linear-gradient(135deg, #f5f0e8, #e8ddd0)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #e0d5c8' }}>
                  <span style={{ fontSize: 20 }}>{['🏖️', '🏰', '🕍'][i]}</span>
                </div>
              ))}
            </div>
            <div style={{ fontSize: 11, color: '#6a5040', lineHeight: 1.6, marginBottom: 16 }}>
              {locale === 'en' ? (
                <>
                  <div>One watercolor per event, based on the venue from step 3</div>
                  <div>Choose or regenerate until you love it</div>
                  <div>Displayed as a decorative background on each card</div>
                </>
              ) : (
                <>
                  <div>Une aquarelle par événement, basée sur le lieu de l&apos;étape 3</div>
                  <div>Choisissez ou régénérez jusqu&apos;à satisfaction</div>
                  <div>Affichée en fond décoratif sur chaque carte</div>
                </>
              )}
            </div>
            <div style={{ fontFamily: 'var(--font-playfair-display)', fontSize: 28, fontWeight: 700, color: '#C9A84C', marginBottom: 4 }}>
              99 &euro;
            </div>
            <div style={{ fontSize: 10, color: '#9a928a', marginBottom: 16 }}>
              {locale === 'en' ? 'One-time payment — included in the Premium pack' : 'Paiement unique — inclus dans le pack Premium'}
            </div>
            <div style={{ display: 'inline-block', padding: '12px 32px', borderRadius: 9999, background: '#e8e0d8', color: '#9a928a', fontSize: 13, fontWeight: 700, letterSpacing: '0.03em' }}>
              {locale === 'en' ? 'Available soon' : 'Bientôt disponible'}
            </div>
          </div>
        </AccordionSection>
                  ...
                  <div style={{ position: 'relative', zIndex: 1, opacity: data.textOpacity ?? 1, textShadow: readableShadow(theme, usePhotoBg, hasFrame), transform: data.textOffsetY ? `translateY(${data.textOffsetY}px)` : undefined }}>
                    ...
                    {(ceremony.type === 'Cérémonie religieuse / Houppa' || ceremony.type === 'Mairie') && (
                      <DraggableElement id={pre+"prenoms"} layout={layout} onLayoutChange={setLayout} editable={canEdit}><AnimSection animStyle={anim} delay={250} skipAnim={canEdit}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'center', gap: 'clamp(8px,2vw,14px)', flexWrap: 'wrap', marginBottom: 4 }}>
